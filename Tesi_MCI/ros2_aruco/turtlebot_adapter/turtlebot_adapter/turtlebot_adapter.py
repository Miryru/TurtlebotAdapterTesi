...
class TurtleBotAdapter(Node):
    def __init__(self , IP , port): #self , session ,useGoogleKey ,IP ,port
        ...
        self.action_client1 = ActionClient(self, NavigateToPose, 'robot4/navigate_to_pose')
        self.action_client1.wait_for_server( timeout_sec=10.0)
        self.marker_pose_subscriber = self.create_subscription(ArucoMarkers, '/robot4/aruco_markers', self.marker_pose_callback, 10)
        
        self.cmd_vel_publisher = self.create_publisher(Twist, '/robot4/cmd_vel', 10)
        self.ack_pub = self.create_publisher(Bool, '/robot4/ack', 10)
        self.ack = False
        self.marker = None
